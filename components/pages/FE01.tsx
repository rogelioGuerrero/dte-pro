...
import { redondear, numeroALetras } from '../../utils/formatters';
...
export const FE01: React.FC = () => {
  ...
  const request = useMemo(() => {
    ...
    try {
      ...
      return { error: error instanceof Error ? error.message : 'No se pudo construir el payload FE 01' };
    }
  }, [ambiente, businessId, emisor, receptorEmail, validItems]);
  ...
  const addLine = () => setLineas((current) => [...current, createLine()]);
  const removeLine = (index: number) => setLineas((current) => current.filter((_, i) => i !== index));

  const handleSend = async () => {
    if (!request || 'error' in request) {
      addToast(builderError || 'Completa el formulario FE 01.', 'error');
      return;
    }
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
          ...
          {lineas.map((linea, index) => (
            <div key={index} className="grid gap-3 rounded-2xl border border-gray-200 p-4 md:grid-cols-[1.4fr,0.6fr,0.6fr,0.6fr,auto]">
              ...
              <input
                ...
                onChange={(e) => updateLine(index, { cantidad: Number(e.target.value) })}
                className="rounded-xl border border-gray-300 px-3 py-2"
              ...
              <input
                ...
                onChange={(e) => updateLine(index, { precioUnitario: Number(e.target.value) })}
                className="rounded-xl border border-gray-300 px-3 py-2"
                placeholder="Precio"
              />
              <input
                ...
                onChange={(e) => updateLine(index, { descuento: Number(e.target.value) })}
                className="rounded-xl border border-gray-300 px-3 py-2"
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-xl border border-gray-300 px-3 py-2 text-gray-600 disabled:opacity-40"
              >
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={addLine}
            className="inline-flex items-center gap-2 rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700"
          >
          ...
          <button
            type="button"
            onClick={handleSend}
            disabled={!request || 'error' in (request || {}) || isSending}
