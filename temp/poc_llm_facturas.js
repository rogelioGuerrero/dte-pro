...
(async () => {
  // ── 1. Leer API Key ──────────────────────────────────────────────────────────
  const settings = JSON.parse(localStorage.getItem('dte_app_settings') || '{}');
  let apiKey = settings.apiKey || '';

  if (!apiKey) {
    apiKey = prompt('🔑 Ingresa tu Gemini API Key (Google AI Studio → aistudio.google.com/apikey):') || '';
    if (!apiKey) {
    ...
    localStorage.setItem('dte_app_settings', JSON.stringify(settings));
    console.log('✅ API Key guardada en localStorage para futuras pruebas.');
  } else {
  ...
  if (dtes.length === 0) {
    console.warn('⚠️ No hay DTEs en cache local. Asegúrate de haber emitido al menos una factura.');
    return;
  }

  console.log(`📄 ${dtes.length} DTE(s) encontrados en cache.`);
